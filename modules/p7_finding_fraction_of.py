...
def find_fraction_of(div_clicks, mul_clicks, frac_str, quant_str):
    ...
    if button_id == 'p7-divide-first-btn':
        step1 = quantity / den
        result = step1 * num
        calc = f"$ ( {currency_symbol}{quantity} \div {den} ) \times {num} = {currency_symbol}{step1:.2f} \times {num} = {currency_symbol}{result:.2f} $"
        return dcc.Markdown(calc)
    elif button_id == 'p7-multiply-first-btn':
        step1 = quantity * num
        result = step1 / den
        calc = f"$ ( {currency_symbol}{quantity} \times {num} ) \div {den} = {currency_symbol}{step1:.2f} \div {den} = {currency_symbol}{result:.2f} $"
        return dcc.Markdown(calc)
